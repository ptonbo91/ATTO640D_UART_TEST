...
-------------------------------
entity DPRAM_GENERIC_DC is
-------------------------------
  Generic (
    ADDR_WIDTH  :     Positive :=  8;           -- RAM Address Width
    DATA_WIDTH  :     Positive := 36;           -- RAM Data Width
    ...
    BYPASS_RW   :     Boolean  := false;        -- Returned Write Data when Read and Write at same address
    SIMPLE_DP   :     Boolean  := false;        -- When set, it enables only the A_write port & B_read port, optimize on Lattice devices
    SINGLE_CLK  :     Boolean  := false;        -- Advertise that A_CLK = B_CLK
...
-------------------------------
begin
  assert ((SIMPLE_DP and not BYPASS_RW) or (not SIMPLE_DP) or (SINGLE_CLK)) report "DPRAM_GENERIC_DC : BYPASS_RW must be FALSE when SIMPLE_DP is TRUE if A_CLK /= B_CLK" severity error;
end entity DPRAM_GENERIC_DC;
-------------------------------


-------------------------------------------
architecture RTL of DPRAM_GENERIC_DC is
-------------------------------------------
